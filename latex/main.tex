...
\address{Institute of Mathematics and Statistics, University of Tartu, Narva
mnt 18, 51009 Tartu, Estonia}
\email{kaarelaugustkurik@gmail.com}

\subjclass[2020]{46B20, 47H09, 05C69}
...
\begin{abstract}
    ...
    $B_{X'} \to B_{X'}$ for some Banach space $X'$, and that a homeomorphic 1-Lipschitz non-isometry of
...
$\gamma_i (x) =
\begin{cases}\norm{x} \ihom_i (\frac{x}{\norm{x}}) &\text{if}\ x \neq 0\\ 0 &\text{otherwise}\end{cases}$, and $\phi_i \colon B_{\sigma(i)} \to B_i$ as $\phi_i = \gamma_i^{-1}$. We additionally define
